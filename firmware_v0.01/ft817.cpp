  ...
  switch (cmd[4]) {
    ...
    case 0x08: // PTT On
      if (!inTx) {
        ...
        digitalWrite(RX, 0); // RX switch off
        si5351.output_enable(SI5351_CLK1, 0); // RX clock off
        si5351.output_enable(SI5351_CLK0, 1); // TX clock on
      } else {
        response[0] = 0xf0;
      }
      Serial.write(response, 1);
    ...
    case 0x88: // PTT OFF
      if (inTx) {
        inTx = 0;
        digitalWrite(RX, 1);
        si5351.output_enable(SI5351_CLK1, 1); // RX clock on
        si5351.output_enable(SI5351_CLK0, 0); // TX clock off
      }
      response[0] = 0;
      Serial.write(response, 1);
      Serial.flush();
