...
/*
   This function is called when the IRQ is fired by the state machine.  See
   enable_pio_isrs for how to register this function to be called.
*/
void IRQ_handler() {
...
/*
   ...
   In freqPIO.pio we enable irq0. This tells the state machine to send the ISRs
   to the core, we still need to tell the core to send them to our program.
*/
void enable_pio_isrs() {
  // Set the function that will be called when the PIO IRQ comes in.
  /*
     Define the local IRQ handler for interrupts fired by the PIO firmware the
     interrupt is honored allowing the firmware to flow despite if the
...
/*
   Main initialization, called from PDX system setup if the FSK_FREQPIO mode is
   enabled
*/
void PIO_init() {
  /*
